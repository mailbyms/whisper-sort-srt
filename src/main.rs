...
// 每行时长超过2秒：可以截断分行。同理，当两个单词间隔（静音）达到2秒，则应该截断分行。
...
struct SubtitleLine {
    ...
    start_time: f64,
    end_time: f64,
    silence_break: bool, // 标记是否因为后面的静音，需要强制换行
}
...
fn split_text_by_punctuation(words: &[Word]) -> Vec<SubtitleLine> {
    ...
    for (i, word) in words.iter().enumerate() {
        ...
        let comming_silence_break = if i + 1 < words.len() {
            ...
            //println!("word:{}, time_gap：{}", &word.word, time_gap);
            time_gap > LINE_MIN_DURATION
        } else {
            //println!("word:{}, time_gap：-",  &word.word);
            false
        };
        ...
        if (word.word.chars().any(|c| punctuation.contains(&c))
            && (char_count >= LINE_MIN_WORD_LENGTH || current_duration >= LINE_MIN_DURATION))
            || ((char_count >= LINE_MAX_WORD_LENGTH || current_duration >= LINE_MAX_DURATION)
                && !is_number
                && word_tokens[i])
            || comming_silence_break
        {
            result.push(SubtitleLine {
                text: filter_filler_words(&current_line.trim().to_string()),
                start_time: current_start,
                end_time: word.end,
                silence_break: comming_silence_break,
            });
 ...
 */
fn match_segments(
    token_segments: &mut Vec<&str>,
    word_segments: &[Word],
    word_tokens: &mut [bool],
) {
...
fn merge_subtitles(blocks: Vec<SubtitleLine>) -> Vec<SubtitleLine> {
    ...
    while i < blocks.len() {
        ...
        merged_blocks.push(SubtitleLine {
            ...
            silence_break: current.silence_break,
        });
